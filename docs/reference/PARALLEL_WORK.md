...
| Tests | none | `test-parallel-work-scripts.sh` (N checks, bash 3.2 + 5, ubuntu + macos), banner cases, `run-all.sh` accumulation |
...
|---|---|---|---|
| `branch.<b>.exosuitParent` | `new-worktree.sh` (unless `--no-parent`) | `worktree-status.sh` (roster, `--me`, every gate), `stream-cleanup.sh`, `merge-up-run.sh`, the `Stream:` banner, `/sprint-end` (`--gate children`) | with the branch: `git branch -d` in cleanup drops the whole `branch.<b>` section. Residue after a hand deletion is reported by `--gate children` as `branch GONE, config residue` with the `git config --remove-section branch.<b>` line to run |
| `branch.<b>.exosuitStory` | `new-worktree.sh --story <id>` (only when a parent is recorded) | roster Story column, `--me story:`, the banner, the HELLO body | with the branch, as above. One story id; free text, control characters stripped and cut at 200 characters at every read; a `+` inside is opaque. Re-point a finished stream by hand: `git config branch.<b>.exosuitStory '<id>'` |
...
| `EXOSUIT_WORKTREE_LAUNCH_CMD` | `claude` | the **base** command the launcher runs in each terminal; `--name '<branch>'` and `-- '<prompt>'` are appended to it | empty: no command (do not do this; see Breaking changes in the CHANGELOG for the 3.0.1 migration) |
...
| `EXOSUIT_WORKTREE_TABS` | `1` | open one terminal per stream | `0`: print the per-directory commands only, exit 0 |
...
| `Stream: <b> <- parent <p> …` | the SessionStart banner inside a stream (hook stdout, ~45 tokens, re-emitted on resume, `/clear`, compaction and fork) | context only; `behind <n> — run /merge-down` is the cue for `/merge-down` | — |
...
| Cleanup never removes an unmerged, dirty, unreadable, parentless, parent-less-worktree, upstream-blocked, current or live stream, and never force-deletes | script refusal: the `CLEANUP: keep` ladder, dry run and `--apply`; the only deletion flag is `-d`, run in the parent's worktree; a refusal is `CLEANUP: FAILED` (exit 1), never retried, never forced | the keep line's reason verbatim; the live-session keep fires only when `claude agents --json` is readable — otherwise the stderr ADVISORY says so and the cwd guard is what remains |
...
| Read-only on the parent: no `git -C <parent_dir>` command, ever (md 4) | prose rule (`--gate merge-down` ignores the parent's state for the same reason) | nothing in the merge-down transcript runs in the parent directory |
